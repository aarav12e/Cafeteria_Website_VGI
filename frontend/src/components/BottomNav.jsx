...
const BottomNav = () => {
    ...
    // Hide on auth pages
    if (pathname === '/auth' || pathname === '/vgi-secret-admin') return null;
    ...
    return (
        <div className="fixed bottom-0 left-0 w-full z-50 md:hidden">
            <div className="px-2 py-2 flex justify-around items-center bg-[#0a0a0a] border-t border-white/10"
                style={{ backdropFilter: 'blur(20px)' }}>
                {navItems.map((item) => {
                    const isActive = pathname === item.path || (item.path !== '/' && pathname.startsWith(item.path));
                    return (
                        <Link key={item.label} to={item.path} className="flex flex-col items-center py-1.5 px-4 group">
                            <div className={`relative p-2 rounded-xl transition-all duration-200 ${isActive ? 'bg-white/10 text-white' : 'text-gray-500 group-hover:text-gray-300'}`}>
                                <span>
                                    {item.icon}
                                </span>
                                {item.badge > 0 && (
                            ...
                            <span className={`text-[10px] mt-0.5 font-semibold transition-colors ${isActive ? 'text-white' : 'text-gray-500 group-hover:text-gray-300'}`}>
                                {item.label}
                            </span>
                        </Link>
                    );
