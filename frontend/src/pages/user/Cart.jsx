...
const Cart = () => {
    ...
    const handleCheckout = async () => {
        ...
        try {
            const orderData = { items: cartItems.map(i => ({ menuId: i._id.split('-')[0], qty: i.qty, name: i.name, price: i.price })), totalAmount };
    ...
    if (cartItems.length === 0) return (
        <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="w-24 h-24 bg-[#0a0a0a] rounded-full flex items-center justify-center mb-6 border border-white/10">
                <FaShoppingBag className="text-gray-600" size={36} />
            </div>
            <h2 className="text-2xl font-black text-white mb-2">Your cart is empty</h2>
            <p className="text-gray-400 mb-6 text-sm">Add some amazing dishes from our menu</p>
            <button onClick={() => navigate('/menu')}
                className="flex items-center gap-2 bg-white text-black px-8 py-3 rounded-full font-bold shadow-lg hover:bg-gray-200 transition-all">
    ...
    return (
        <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-[#0a0a0a] rounded-3xl p-5 mb-4 space-y-4 border border-white/10">
                <AnimatePresence>
                    {cartItems.map(item => (
                        <motion.div key={item._id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            ...
                            <div className="flex items-center gap-2">
                                <div className="flex items-center bg-[#111] rounded-xl border border-white/10 overflow-hidden">
                                    ...
                                    <button onClick={() => updateQty(item._id, item.qty + 1)} className="px-2.5 py-2 text-gray-500 hover:text-white hover:bg-white/10 transition"><FaPlus size={10} /></button>
                                </div>
                                <button onClick={() => removeFromCart(item._id)} className="p-2 text-gray-500 hover:text-red-400 hover:bg-red-900/20 rounded-xl transition"><FaTrash size={13} /></button>
                            </div>
                        </motion.div>
                    ))}
                </AnimatePresence>
            ...
            <div className="bg-[#0a0a0a] rounded-3xl p-5 border border-white/10">
                ...
                <button onClick={handleCheckout}
                    className="mt-4 w-full bg-white text-black py-4 rounded-2xl font-black text-base shadow-xl hover:bg-gray-200 transition-all flex items-center justify-center gap-2">
