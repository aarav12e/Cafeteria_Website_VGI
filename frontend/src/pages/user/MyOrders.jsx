    ...
    return (
        <div className="max-w-2xl mx-auto">
            <h2 className="text-2xl font-black text-white mb-6 flex items-center gap-2">
                <FaClipboardList className="text-white" /> My Orders
            </h2>
            ...
            {orders.length === 0 ? (
                ...
                <div className="space-y-4">
                    {orders.map(order => (
                        <div key={order._id} className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-5 shadow-lg">
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <p className="text-xs text-gray-500 font-medium">Order ID</p>
                                    <p className="text-sm font-black text-white font-mono">#{order._id.slice(-8).toUpperCase()}</p>
                                    <p className="text-xs text-gray-500 mt-0.5">{new Date(order.createdAt).toLocaleString('en-IN')}</p>
                                </div>
                                <div className="flex flex-col gap-1.5 items-end">
                                    <span className={`px-3 py-1 rounded-full text-[10px] uppercase tracking-widest font-bold border ${STATUS[order.orderStatus] || 'bg-white/10 text-white border-white/20'}`}>
                                        {order.orderStatus.toUpperCase()}
                                    </span>
                                    <span className={`px-3 py-1 rounded-full text-[10px] uppercase tracking-widest font-bold border ${order.paymentStatus === 'paid' ? 'bg-white/10 text-white border-white/20' : 'bg-red-900/50 text-red-200 border-red-900/50'}`}>
                            ...
                            <div className="space-y-1 border-t border-b border-white/10 py-3 mb-3">
                                {order.items.map((item, i) => (
                                    <div key={i} className="flex justify-between text-sm text-gray-400">
                                        ...
                                        <span className="font-semibold text-gray-300">₹{item.price * item.qty}</span>
                                    </div>
                                ))}
                            </div>
